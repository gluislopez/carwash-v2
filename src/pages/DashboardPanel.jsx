            ...
            <div className="dashboard-header" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '2rem' }}>
                <div>
                    <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '0.5rem' }}>
                        ...
                        <span style={{ fontSize: '0.8rem', color: 'white', backgroundColor: '#25D366', border: '1px solid white', padding: '0.2rem 0.5rem', borderRadius: '4px', boxShadow: '0 0 10px #25D366' }}>
                            v4.148 WHATSAPP FIX {new Date().toLocaleTimeString()}
                        </span>
                    </div>
                    ...
                    <button
                        onClick={async () => {
                            try {
                                alert('Iniciando prueba de subida...');
                                const blob = new Blob(['Hello World'], { type: 'text/plain' });
                                const fileName = `test_${Date.now()}.txt`;
                                const { data, error } = await supabase.storage.from('receipts').upload(fileName, blob);
                                if (error) throw error;
                                alert('¡ÉXITO! El sistema de archivos funciona. El problema es el PDF.');
                            } catch (e) {
                                alert('FALLÓ LA PRUEBA: ' + (e.message || JSON.stringify(e)));
                            }
                        }}
                        style={{
                            marginTop: '10px',
